...
| # | Item | Detail |
|---|---|---|
| A1 | **Agent bridge (MCP server) for the live editor/engine** | Let *any* agent — not just the built-in panel — drive a running instance: spawn/modify entities, start/stop Play, query world state and logs, capture viewport screenshots, trigger compiles/bakes. Turns "agent-friendly files" into "agent-operable engine"; a demo no incumbent can match. |
| A2 | **The proof video** | One prompt → playable mini-game in ~10 unedited minutes: agent authors JSON, runs headless, reads logs, screenshots, iterates. The engine can already do this today — this is the marketing asset. |
| A3 | **Genre starter kits** (platformer, top-down, FPS) | Matter double: onboarding for humans *and* few-shot examples for agents. Ship as Hub templates with full Docs/. |
| A4 | **Agent-grade error reporting** | Schema validation with precise messages (`file:node:pin`) instead of silent `[warn] unknown ...`; a single `--verify` command that runs resave + N frames + log-scan and exits nonzero on failure; machine-readable (JSON) stat/log output. |
| A5 | **Live PulseLABS run** of the AI Assistant (config-only step per the smoke test), then multi-turn proposal refinement + auto-compile of proposed C++. |
| A6 | **Screenshot diffing helper** | `--screenshot` + a reference-compare mode (pixel/PSNR budget) so agents can assert visual regressions headlessly — the engine already A/Bs renders internally; expose it. |
...
| # | Item | Detail |
|---|---|---|
| C1 | Animation: blend trees / 2D blending, layers, root motion, IK, morph targets; per-instance animation of shared models (currently one animated instance per asset). |
| C2 | Particles: texture/flipbook sprites, per-particle rotation, soft depth fade, GPU sim for big counts. |
| C3 | Material graphs: normal-map output, more texture slots, subgraphs/functions. |
